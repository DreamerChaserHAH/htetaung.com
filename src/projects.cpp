...
Projects::Projects(){
    ...
    windowItems[1] = WindowItem{"Multi-Chess (2024)", "A C++ Multiplayer Chess Game running inside the browser through WebRTC and WASM technologies", LoadTexture("resources/sprites/MultiChessProject.png")};
    ...
    windowItems[5] = WindowItem{"Tank Crews (2020)", "A Unity Multiplayer Tank Game using Photon SDK with a huge emphasis on collaboration over voice chat", LoadTexture("resources/sprites/TankCrewsProject.png")};
...
void Projects::DrawProjects(){
    DrawHeader("PROJECTS", Vector2{230, 110}, 64);
    parentMenu->DrawMenu(100);
    parentMenu->UpdateMenu();
    simpleWindow.DrawSimpleWindow();
}